...
def get_url(i, referer_url, api_urls_queue, api_url, headers, folder_path, api_url_res, filePath_url_info, prefixes=None, current_depth=0):
    try:
        ...
        res = requests.get(url=api_url, headers=headers, timeout=TIMEOUT, verify=False, allow_redirects=False, proxies=proxies)
        ...
        file_path = ""
        rid = record_req_resp_unified(DB_WRITER, RR_CONFIG, folder_path, api_url, 'GET', dict(res.request.headers), headers.get('Cookie',''), text, res_code, res_type, True, referer_url, file_path, None, dict(res.headers))
        # OOM优化: 不在内存中存储完整的 response，仅保留文件路径
        api_url_res.append({"url": api_url, 'method': 'GET', "res_type": res_type, "res_code": res_code, 'res_size': len(text), 'referer_url': referer_url, 'file_path': file_path, 'parameter': "", "response": "", 'request_id': rid})
...
def post_data_url(i, referer_url, api_urls_queue, api_url, headers, folder_path, api_url_res, filePath_url_info, prefixes=None, current_depth=0):
    try:
        ...
        res = requests.post(url=api_url, data="", headers=h, timeout=TIMEOUT, verify=False, allow_redirects=False, proxies=proxies)
        res_code, res_type, text = res.status_code, res.headers.get('Content-Type', ""), res.text
        ...
        file_path = ""
        rid = record_req_resp_unified(DB_WRITER, RR_CONFIG, folder_path, api_url, 'POST_DATA', dict(res.request.headers), h.get('Cookie',''), text, res_code, res_type, True, referer_url, file_path, "", dict(res.headers))
        api_url_res.append({"url": api_url, 'method': 'POST_DATA', "res_type": res_type, "res_code": res_code, 'res_size': len(text), 'referer_url': referer_url, 'file_path': file_path, 'parameter': "", "response": text, 'request_id': rid})
    except Exception: pass
...
def post_json_url(i, referer_url, api_urls_queue, api_url, headers, folder_path, api_url_res, filePath_url_info, prefixes=None, current_depth=0):
    try:
        ...
        res = requests.post(url=api_url, json={}, headers=h, timeout=TIMEOUT, verify=False, allow_redirects=False, proxies=proxies)
        res_code, res_type, text = res.status_code, res.headers.get('Content-Type', ""), res.text
        ...
        file_path = ""
        rid = record_req_resp_unified(DB_WRITER, RR_CONFIG, folder_path, api_url, 'POST_JSON', dict(res.request.headers), h.get('Cookie',''), text, res_code, res_type, True, referer_url, file_path, "{}", dict(res.headers))
        # OOM优化: 不在内存中存储完整的 response
        api_url_res.append({"url": api_url, 'method': 'POST_JSON', "res_type": res_type, "res_code": res_code, 'res_size': len(text), 'referer_url': referer_url, 'file_path': file_path, 'parameter': "", "response": "", 'request_id': rid})
    except Exception: pass
...
def apiUrlReqNoParameter_api(referer_url, api_path_urls, cookies, folder_path, filePath_url_info, db_path=None, rr_config=None, prefixes=None, url_methods=None, ignore_get_urls=None, scanner=None):
    ...
    # 注入 scanner 到全局或作为参数传递
    # 这里通过 monkey patching 或者修改内部函数签名来传递
    # 简单起见，我们将 scanner 附加到 apiUrlReq 函数的参数中，但需要修改所有内部调用
    # 更好的方式是设置一个全局变量（在当前模块作用域）
