...
def urlFilter(lst):
    ...
    for line in lst:
        ...
        # 2. 增强过滤：过滤纯字母数字且不包含/或.的（除非是常见API路径）
        # 这一步可以有效过滤 httpAgent, httpsAgent 等
        if not line:
            continue
        ...
        # 如果不包含 / 且不以 http 开头，极有可能是误报（变量名）
        if '/' not in line and not line_lower.startswith('http'):
             # 特例：如果虽然不包含/（被strip掉了），但原始字符串是以 api/ 开头的，我们认为是合法的
             # 但因为这里 line 已经被 strip 和 replace 了，可能很难还原
             # 实际上，如果上面正则匹配到了 api/，这里的 line 会是 "api/" 或 "api"，所以我们放行 "api" 这种特例
             if line_lower in ['api', 'v1', 'v2', 'v3']:
                 pass
             else:
                 continue
...
def get_compiled_api_pattern():
    ...
    api_patterns = [
        ...
        # 修复：强制要求 http(s) 后面跟 ://，避免匹配 httpAgent
        ...
        # 修复：href/action 后面的非引号内容也应该比较严格，这里先加上 http 限制或 / 限制可能太严，暂不改动，依靠 urlFilter
        r'(?P<href_action_unquoted>(href|action).{0,3}=.{0,3}[^\s\'\'"\>\<\)\(]{2,250})',
        r'(?P<path_slash>(?:"|\'|`)(\/[^"\'`<>]+)(?:"|\'|`))',
        # 新增：显式支持以 api/ 或 v\d+/ 开头的单层相对路径（如 "api/"）
...
def get_api_path(i, js_urls_queue, js_url, headers, all_api_paths, folder_path, url_to_path=None, db_path=None, url_to_hash=None):
    try:
        text = ""
        # 如果缓存中已存在且该内容哈希已解析过，则直接跳过
        if url_to_path and js_url in url_to_path and os.path.isfile(url_to_path[js_url]):
            try:
                if db_path and url_to_hash and (js_url in url_to_hash):
                    import sqlite3
                    conn = sqlite3.connect(db_path)
                    try:
                        conn.execute("CREATE TABLE IF NOT EXISTS step3_js_parsed (content_hash TEXT PRIMARY KEY)")
                        cur = conn.execute("SELECT 1 FROM step3_js_parsed WHERE content_hash=?", (url_to_hash[js_url],))
                        row = cur.fetchone()
                        if row:
                            # 已解析过，跳过
                            logger_print_content(f"[线程{i}] 剩下:{js_urls_queue.qsize()} {js_url}缓存内容已解析，跳过")
                            conn.close()
                            return
                    except Exception:
                        pass
                ...
                pass
            with open(url_to_path[js_url], 'r', encoding='utf-8', errors='ignore') as f:
                text = f.read()
            logger_print_content(f"[线程{i}] 剩下:{js_urls_queue.qsize()} {js_url}使用缓存JS内容")
        else:
            GlobalRequestCounter.increment()
            try:
                head_precheck = (os.environ.get('CHKAPI_HEAD_PRECHECK', '0').strip() == '1')
            except Exception:
                head_precheck = False
            sample_bytes = 0
            content_length = 0
            if head_precheck:
                try:
                    res_head = requests.head(url=js_url, headers=headers, timeout=TIMEOUT, verify=False, allow_redirects=False, proxies=proxies)
                    if int(res_head.status_code) >= 400:
                        logger_print_content(f"[线程{i}] 剩下:{js_urls_queue.qsize()} {js_url} HEAD预检失败，跳过\t状态码:{res_head.status_code}")
                        return
                    try:
                        content_length = int(res_head.headers.get('Content-Length') or '0')
                    except Exception:
                        content_length = 0
                except Exception:
                    pass
            if sample_bytes and content_length and content_length > sample_bytes:
                h2 = dict(headers or {})
                h2['Range'] = f'bytes=0-{sample_bytes-1}'
                res = requests.get(url=js_url, headers=h2, timeout=TIMEOUT, verify=False, allow_redirects=False, proxies=proxies, stream=True)
                logger_print_content(f"[线程{i}] 剩下:{js_urls_queue.qsize()} {js_url}分段采样\t状态码:{res.status_code}\t长度:{content_length}")
                buf = b''
                try:
    ...
    for match in matches_iter:
        ...
        if ('axios.post' in ctx_low) or ('$.post' in ctx_low) or ('method' in ctx_low and 'post' in ctx_low) or ('type' in ctx_low and 'post' in ctx_low):
            method_hint = 'POST'
        elif ('axios.put' in ctx_low) or ('$.put' in ctx_low) or ('method' in ctx_low and 'put' in ctx_low):
            method_hint = 'PUT'
        elif ('axios.delete' in ctx_low) or ('$.delete' in ctx_low) or ('method' in ctx_low and 'delete' in ctx_low):
            method_hint = 'DELETE'
        elif ('axios.get' in ctx_low) or ('$.get' in ctx_low) or ('fetch' in ctx_low) or ('method' in ctx_low and 'get' in ctx_low):
            method_hint = 'GET'
        ...
        if new_api_path in seen_paths:
            ...
            if context_info:
                all_api_paths.append({
                    ...
                    'url_type': "api_path",
                    'regex_context': context_info,
                    'method': (context_info.get('method') or '').upper() if context_info.get('method') else ''
                })
            else:
                all_api_paths.append({
                    'referer': js_url,
    ...
    try:
        if db_path:
            ...
            conn.execute("CREATE TABLE IF NOT EXISTS step3_js_parsed (content_hash TEXT PRIMARY KEY)")
            try:
                conn.execute("INSERT OR IGNORE INTO step3_js_parsed(content_hash) VALUES(?)", (content_hash,))
                conn.commit()
            except Exception:
                pass
            conn.close()
...
def apiPathFind(i, js_urls_queue, headers, all_api_paths, folder_path, url_to_path=None, db_path=None, url_to_hash=None):
    while not js_urls_queue.empty():
        try:
            js_url = js_urls_queue.get(timeout=1)
            get_api_path(i, js_urls_queue, js_url, headers, all_api_paths, folder_path, url_to_path, db_path, url_to_hash)
        except Exception:
            break

def apiPathFind_api(js_urls, cookies, folder_path, db_path=None):
    logger_print_content(f"js_urls: {js_urls}")
    all_api_paths = []
    url_to_path = {}
    url_to_hash = {}
    # 目标站点与内部主机集合（用于跳过外部CDN）
    target_host = ''
    internal_hosts = set()
    skip_external = True
    try:
        ev = os.environ.get('CHKAPI_SKIP_EXTERNAL')
        if isinstance(ev, str):
            skip_external = (ev.strip() != '0')
    except Exception:
        skip_external = True
    try:
        evh = os.environ.get('CHKAPI_INTERNAL_HOSTS') or ''
        if evh.strip():
            internal_hosts = {h.strip().lower() for h in evh.split(',') if h.strip()}
    except Exception:
        internal_hosts = set()
    if db_path and os.path.isfile(db_path):
        try:
            ...
            conn = sqlite3.connect(db_path)
            try:
                cur0 = conn.execute("SELECT original_url FROM meta_target_info ORDER BY scan_time DESC LIMIT 1")
                row0 = cur0.fetchone()
                if row0 and row0[0]:
                    pu = urlparse(row0[0])
                    target_host = (pu.hostname or '').lower()
                    if target_host:
                        internal_hosts.add(target_host)
            except Exception:
                pass
            cur = conn.execute("SELECT url, path, content_hash FROM step2_js_cache")
            for row in cur.fetchall():
                u, p, h = row[0], row[1], row[2]
                if u and p and os.path.isfile(p):
                    url_to_path[u] = p
                    if h:
                        url_to_hash[u] = h
            # 创建已解析哈希记录表
    ...
    # 使用 ThreadPoolExecutor 进行线程管理
    js_urls_queue = Queue()
    _queued = set()
    try:
        max_js_limit = int(os.environ.get('CHKAPI_MAX_JS', '0'))
    except Exception:
        max_js_limit = 0
    skip_vendors = False
    queued_count = 0
    for raw in js_urls:
        js_url = (raw or '').strip()
        # 清理列表里可能出现的反引号和包裹引号
        if js_url and (js_url[0] in "\"'`") and (js_url[-1:] in "\"'`"):
            js_url = js_url[1:-1]
        js_url = js_url.strip()
        # 某些来源包含空格和反引号包裹
        ...
        # 跳过 vendors 资源
        try:
            base = os.path.basename(urlparse(js_url).path or '')
        except Exception:
            base = ''
        if skip_vendors and base and ('vendors' in base or base.startswith('chunk-vendors')):
            logger_print_content(f"[跳过vendors] {js_url}")
            continue
        # 跳过外部CDN以避免阻塞（可通过环境变量关闭）
        try:
            pu = urlparse(js_url)
            host = (pu.hostname or '').lower()
        except Exception:
            host = ''
        if skip_external and host and (host not in internal_hosts):
            logger_print_content(f"[跳过外部JS] {js_url}")
            continue
        if js_url and (js_url not in _queued):
            _queued.add(js_url)
            if (max_js_limit and queued_count >= max_js_limit):
                continue
            js_urls_queue.put(js_url)
            queued_count += 1

    try:
        env_workers = int(os.environ.get('CHKAPI_WORKERS', '0'))
    except Exception:
        env_workers = 0
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for i in range(max_workers):
            futures.append(executor.submit(apiPathFind, i, js_urls_queue, headers, all_api_paths, folder_path, url_to_path, db_path, url_to_hash))
