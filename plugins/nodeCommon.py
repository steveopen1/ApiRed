...
# Reuse HTTP sessions across the tool to avoid repeatedly creating TCP connections
_REQUEST_SESSION_CACHE = {}
_REQUEST_SESSION_LOCK = threading.Lock()
_REQUEST_POOL_SIZE = 100

def _session_cache_key(proxies=None):
    if not proxies:
        return 'default'
    if isinstance(proxies, dict):
        return 'dict:' + ';'.join(f"{k}={v}" for k, v in sorted(proxies.items()))
    return f"str:{proxies}"

def _build_session(proxies=None):
    session = requests.Session()
    adapter = HTTPAdapter(pool_connections=_REQUEST_POOL_SIZE, pool_maxsize=_REQUEST_POOL_SIZE, max_retries=3)
...
def get_http_session(proxies=None):
    key = _session_cache_key(proxies)
    session = _REQUEST_SESSION_CACHE.get(key)
    if session:
        return session
    with _REQUEST_SESSION_LOCK:
        session = _REQUEST_SESSION_CACHE.get(key)
        if session:
            return session
        session = _build_session(proxies)
        _REQUEST_SESSION_CACHE[key] = session
        return session

def http_request(method, url, **kwargs):
    proxies = kwargs.pop('proxies', None)
    session = get_http_session(proxies)
    # Copy headers to prevent cross-request mutation
...
class DatabaseWriter:
    ...
    def _commit(self, conn, reqs, resps, files):
        try:
            ...
            if resps: conn.executemany("INSERT INTO response_log(id,url,method,res_code,res_type,response,timestamp,format,file_path,is_no_param) VALUES(?,?,?,?,?,?,?,?,?,?)", resps)
...
def record_req_resp_unified(db_writer, config, folder_path, api_url, method, headers_map, cookies, text, res_code, res_type, is_no_param, referer_url, file_path, body=None):
    """
    ...
    try:
        ...
        fmt = 'json' if ('json' in str(res_type).lower()) else ('xml' if ('xml' in str(res_type).lower()) else 'text')
        ...
        if db_writer:
            ...
            db_writer.add_resp((rid, log_url, method, int(res_code), str(res_type or ''), text or "", ts, fmt, log_url, 1 if is_no_param else 0))
            
            # 响应包去重逻辑 (使用哈希进行增量记录)
            if text:
                h = hashlib.sha256((text).encode("utf-8", errors="ignore")).hexdigest()
                # 这里的 file_path 字段我们直接存源 URL
                db_writer.add_file((method, log_url, log_url, h, len(text), rid))
        return rid
